		            ...
		            if (result == DialogResult.Yes)
		            {
		                ...
		                try
		                {
		                    using (MySqlConnection cn = new MySqlConnection("server=localhost; user id=root; password=; database=cafems;"))
		                    {
		                        ...
		                        string query = "SELECT COUNT(*) FROM admin WHERE (name = @name OR username = @user OR password = @pass) AND ID != @id";
		                        using (MySqlCommand checkCommand = new MySqlCommand(query, cn))
		                        {
		                            ...
		                            checkCommand.Parameters.AddWithValue("@pass", txtPass.Text);
		                            checkCommand.Parameters.AddWithValue("@id", selectedID);
		(checkCommand.ExecuteScalar());
		                            if (count > 0)
		                            int count = Convert.ToInt32
		                            {
		                                MessageBox.Show("Name, username, or password already exists. Please choose different values.");
		                                txtclear();
		                                return;
		                            }
		                        }
